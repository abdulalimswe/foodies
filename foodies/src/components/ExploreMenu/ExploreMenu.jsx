           ...
           <div className="d-flex justify-content-between gap-4 overflow-auto explore-menu-list" ref={menuRef}>
               {
                   categories.map((item,index) => {
                          return (
                              <div key={index} className='text-center explore-menu-list-item' onClick={() => setCategory(prev => prev === item.name ? 'All' : item.name)}>
                                  <img src={item.image} alt="" className={item.name === category ? 'rounded-circle active' : "rounded-circle"} height={128} width={128}/>
